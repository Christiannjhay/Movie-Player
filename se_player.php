...
// doodstream = 21, streamtape = 25, streamsb = 26, voe = 29, ninjastream = 33, vipstream = 0
...
if(isset($_GET['video_id'])) { 
    ...
    if(!empty(trim($video_id))) { 
        ...
        if(!empty($player_url)) { 
            $player_data = json_decode($player_url, true);
            if (isset($player_data['data']['sources'])) {
                echo "<h2>Available Servers and Qualities</h2><ul>";
                foreach ($player_data['data']['sources'] as $source) {
                    echo "<li>Server: " . htmlspecialchars($source['server']) . " - Quality: " . htmlspecialchars($source['quality']) . "</li>";
                }
                echo "</ul>";
            } else {
